...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "dzeck-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('dzeck_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('dzeck_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("dzeck_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface DzeckClientOptions {
  url: string;
  reconnect?: boolean;
...
export class DzeckClient {
  ...
  constructor(private options: DzeckClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[dzeck ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - DzeckClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
