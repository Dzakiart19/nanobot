  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/dzeck-bench",
          project_name: "dzeck-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const dzeckSection = screen.getByRole("region", { name: "dzeck" });
    const dzeckText = dzeckSection.textContent ?? "";

    expect(screen.getByRole("region", { name: "dzeck-bench" })).toBeInTheDocument();
    expect(within(dzeckSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(dzeckSection).getByText("Zeta task")).toBeInTheDocument();
    expect(dzeckText.indexOf("Alpha task")).toBeLessThan(dzeckText.indexOf("Zeta task"));
    expect(within(dzeckSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace chats in the Chats section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/.dzeck/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.dzeck/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "dzeck" })).toBeInTheDocument();
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/dzeck": "Photos" }}
        collapsedGroups={{ "project:/Users/me/dzeck": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/dzeck");
    ...
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/dzeck", "Photos");
    ...
    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/dzeck", "Photos");
  });
