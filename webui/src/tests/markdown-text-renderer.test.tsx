...
describe("MarkdownTextRenderer", () => {
  ...
  it("does not wrap complete fenced code blocks in an extra pre", () => {
    const { container } = render(
      <MarkdownTextRenderer highlightCode={false}>
        {"当前目录:\n\n```text\n/Users/renxubin/.dzeck/workspace\n```"}
      </MarkdownTextRenderer>,
    );

    expect(screen.getByText("/Users/renxubin/.dzeck/workspace")).toBeInTheDocument();
  ...
  it("renders markdown videos as inline players", () => {
    render(<MarkdownTextRenderer>![dzeck-intro.mp4](/api/media/sig/video)</MarkdownTextRenderer>);

    const video = screen.getByLabelText("Video attachment: dzeck-intro.mp4");
    expect(video.tagName).toBe("VIDEO");
    expect(video).toHaveAttribute("src", "/api/media/sig/video");
    expect(video).toHaveAttribute("controls");
    expect(screen.queryByRole("img", { name: "dzeck-intro.mp4" })).not.toBeInTheDocument();
  });
