...
/**
 * Minimal fake WebSocket implementing the subset DzeckClient touches.
...
afterEach(() => {
  Reflect.deleteProperty(window, "dzeckHost");
  vi.useRealTimers();
});

describe("DzeckClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("dzeck-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("dzeck-host://engine/");
    expect(lastSocket().url).toBe("host:dzeck-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    Object.defineProperty(window, "dzeckHost", {
      configurable: true,
      value: {
        openSocket,
    ...
    expect(openSocket).toHaveBeenCalledWith("dzeck-host://engine/");
