...
The browser front-end for the dzeck gateway. It is built with Vite + React 18 +
...
If you installed dzeck via `pip install dzeck-ai`, the WebUI is **already bundled** in the wheel. Enable the WebSocket channel in `~/.dzeck/config.json` and run `dzeck gateway` — see the root [`README.md`](../README.md#-webui) for the 3-step setup. You do **not** need anything in this directory.
...
dzeck/web/dist/      build output served by the gateway
```

## Develop the WebUI (Vite HMR)

### 1. Install dzeck from source
...
```bash
DZECK_API_URL=http://127.0.0.1:9000 bun run dev
```

### Access from another device (LAN)

To use the WebUI from another device on the same network, set `host` to `"0.0.0.0"` and configure a `token` or `tokenIssueSecret` in `~/.dzeck/config.json`:
...
```bash
cd webui
bun run build          # writes to ../dzeck/web/dist
```
