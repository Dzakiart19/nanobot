...
from dzeck.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
from dzeck.bus.queue import MessageBus
from dzeck.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
    _parse_envelope,
...
from dzeck.session import webui_turns as wth
from dzeck.session.manager import SessionManager
from dzeck.webui.gateway_services import GatewayServices, build_gateway_services
from dzeck.webui.http_utils import (
...
from dzeck.webui.settings_api import settings_payload, update_provider_settings
...
def isolate_webui_workspace_state(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        "dzeck.webui.workspaces.get_webui_dir",
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-Dzeck-Auth", "my-secret")])
...
async def test_webui_message_envelope_persists_user_transcript_for_refresh(
    ...
    from dzeck.webui.transcript import build_webui_thread_response, read_transcript_lines

    monkeypatch.setattr("dzeck.config.paths.get_data_dir", lambda: tmp_path)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
async def test_webui_stop_control_message_is_not_persisted_as_user_bubble(
    ...
    from dzeck.webui.transcript import read_transcript_lines
...
async def test_webui_user_transcript_append_failure_does_not_block_inbound(
    ...
    monkeypatch.setattr("dzeck.channels.websocket.append_transcript_object", fail_append)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
async def test_send_stages_external_media_as_signed_url(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("dzeck.channels.websocket.get_media_dir", fake_media_dir)
    monkeypatch.setattr("dzeck.webui.media_gateway.get_media_dir", fake_media_dir)
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"]}, bus, gateway=_basic_handler(bus))
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
...
def test_registry_discovers_websocket_channel() -> None:
    from dzeck.channels.registry import load_channel_class
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    try:
        ...
        assert workspace_path.endswith("/.dzeck/workspace")
...
async def test_bootstrap_exposes_native_surface(bus: MagicMock) -> None:
    ...
    try:
        response = await _http_get(
            ...
            headers={"X-Dzeck-Auth": "native-secret"},
        )
        assert response.status_code == 200
        body = response.json()
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("dzeck.config.loader._current_config_path", config_path)
    monkeypatch.setenv("DZECK_SANDBOX_ENFORCED", "macos_app_sandbox")
...
def test_handle_webui_thread_get_returns_json(tmp_path, monkeypatch) -> None:
    ...
    from dzeck.webui.transcript import append_transcript_object
