...
async def test_openai_compat_stream_forwards_reasoning_deltas_deepseek_style() -> None:
    ...
    with patch("dzeck.providers.openai_compat_provider.AsyncOpenAI") as mock_openai:
...
async def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("dzeck.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
    ...
    assert headers["HTTP-Referer"] == "https://github.com/HKUDSLab/dzeck"
    assert headers["X-OpenRouter-Title"] == "dzeck"
...
async def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    headers = mock_client_cls.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://dzeck.ai"
...
async def test_openrouter_keeps_model_name_intact() -> None:
    ...
    with patch("dzeck.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("DZECK_STREAM_IDLE_TIMEOUT_S", "0")
