...
from dzeck.command.builtin import cmd_new, register_builtin_commands
from dzeck.command.router import CommandContext, CommandRouter
from dzeck.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("dzeck.agent.loop.SessionManager"), \
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates dzeck onboard)."""
        from dzeck.config.loader import save_config
