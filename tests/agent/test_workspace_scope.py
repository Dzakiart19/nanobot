...
from dzeck.agent.tools.cli_apps import CliAppsTool
from dzeck.agent.tools.filesystem import ReadFileTool
from dzeck.agent.tools.image_generation import ImageGenerationError, ImageGenerationTool
...
from dzeck.apps.cli.service import CliAppManager, CliAppsRuntimeConfig
from dzeck.config.schema import ImageGenerationToolConfig, ProviderConfig
...
async def test_cli_app_scope_controls_working_dir(
    ...
    monkeypatch.setattr("dzeck.apps.cli.service.get_runtime_subdir", lambda _name: data_dir)
    monkeypatch.setattr(
        "dzeck.apps.cli.service.shutil.which",
    ...
    monkeypatch.setattr("dzeck.apps.cli.service.subprocess.run", fake_run)
    tool = CliAppsTool(
        workspace=tmp_path,
