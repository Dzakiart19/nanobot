...
def _make_loop(tmp_path):
    ...
    with patch("dzeck.agent.loop.ContextBuilder"), \
         ...
         patch("dzeck.agent.loop.SubagentManager") as mock_sub_mgr:
...
async def test_drain_injections_returns_empty_when_no_callback():
    ...
    from dzeck.agent.runner import AgentRunner, AgentRunSpec
...
async def test_injection_cycles_capped_at_max():
    ...
    from dzeck.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from dzeck.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from dzeck.agent.loop import AgentLoop
    from dzeck.agent.runner import _MAX_INJECTIONS_PER_TURN
    from dzeck.bus.events import InboundMessage
    from dzeck.bus.queue import MessageBus
...
async def test_drain_injections_on_empty_final_response():
    ...
    from dzeck.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner, AgentRunSpec
    from dzeck.bus.events import InboundMessage
