...
from dzeck.apps.cli.service import CliAppError, CliAppManager, CliAppsRuntimeConfig
...
def test_payload_includes_dzeck_extension_registry(tmp_path: Path) -> None:
    manager = _manager(tmp_path)
    _write_cache(manager._cache_path("harness"), {"meta": {"updated": "2026-04-16"}, "clis": []})
    ...
    assert app["manifest"]["source"] == "dzeck-extension"
    assert app["manifest"]["trust"]["registry"] == "dzeck-extension"
...
def test_optional_extension_registry_failure_does_not_break_payload(
    ...
    monkeypatch.setattr("dzeck.apps.cli.service.httpx.get", fail_get)
...
def test_install_records_entry_point_path_and_pip_distribution(
    ...
    monkeypatch.setattr(
        "dzeck.apps.cli.service.importlib_metadata.distributions",
        lambda: [
            SimpleNamespace(
                entry_points=[
...
def test_fetch_skill_content_allows_cli_anything_raw_skill_url(
    ...
    monkeypatch.setattr("dzeck.apps.cli.service.httpx.get", fake_get)
