...
from dzeck.dzeck import Dzeck, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Dzeck.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Dzeck.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("dzeck.config.loader.load_config") as mock_load, \
         patch("dzeck.providers.factory.make_provider") as mock_prov:
...
async def test_run_with_hooks(tmp_path):
    from dzeck.agent.hook import AgentHook, AgentHookContext
    from dzeck.bus.events import OutboundMessage
...
def test_workspace_override(tmp_path):
    ...
    bot = Dzeck.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    ...
    assert dzeck.Dzeck is Dzeck
    assert dzeck.RunResult is RunResult
...
async def test_run_populates_tools_used_across_iterations(tmp_path):
    ...
    from dzeck.agent.hook import AgentHookContext
    from dzeck.bus.events import OutboundMessage
    from dzeck.providers.base import ToolCallRequest
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from dzeck.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from dzeck.providers.base import ToolCallRequest
