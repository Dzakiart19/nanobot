...
def _make_loop():
    ...
    with patch("dzeck.agent.loop.ContextBuilder"), \
         ...
         patch("dzeck.agent.loop.SubagentManager"):
...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from dzeck.command.builtin import cmd_restart
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch("dzeck.command.builtin.asyncio", new=fake_asyncio), \
             patch("dzeck.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("dzeck.command.builtin.asyncio", new=fake_asyncio), \
             patch("dzeck.command.builtin.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            assert "dzeck" in out.content.lower() or "Model" in out.content
