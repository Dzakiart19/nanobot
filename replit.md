...
- `nanobot/` — Core Python package (agent loop, channels, providers, tools)
...
- **Backend (Gateway)**: `nanobot gateway` on port 8080 (health/status endpoint)
- **WebSocket Channel**: `nanobot` websocket channel on port 8081 (serves WebUI HTTP + WebSocket)
...
2. **Backend Gateway** — `nanobot gateway --port 8080` (backend)

## Configuration

The nanobot config is at `~/.nanobot/config.json`. On first run it's configured with Ollama (local LLM) as a placeholder. Users can configure their preferred LLM provider (OpenAI, Anthropic, etc.) through the Settings UI in the WebUI.
