...
Dzeck is a lightweight, open-source AI agent framework written in Python with a React/TypeScript WebUI. It centers around a small agent loop that receives messages from chat channels, invokes an LLM provider, executes tools, and manages session memory.
...
# Build outputs to ../Dzeck/web/dist (bundled into the Python wheel)
cd webui && bun run dev      # or DZECK_API_URL=... bun run dev
...
- **Skills** (`Dzeck/skills/`): Built-in skill definitions (long-goal, cron, github, image-generation, etc.) loaded into agent context.
- **Security** (`Dzeck/security/`): PTH file guard and other security measures activated at CLI entry.
