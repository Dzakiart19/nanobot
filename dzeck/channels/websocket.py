"""WebSocket server channel: Dzeck acts as a WebSocket server and serves connected clients."""
...
from dzeck.config.paths import get_media_dir
from dzeck.config.schema import Base
from dzeck.security.workspace_access import (
    WORKSPACE_SCOPE_METADATA_KEY,
    WorkspaceScopeError,
)
from dzeck.session.goal_state import goal_state_ws_blob
from dzeck.session.webui_turns import websocket_turn_wall_started_at
from dzeck.utils.media_decode import (
    FileSizeExceeded,
    save_base64_data_url,
)
from dzeck.webui.cli_apps_api import normalize_cli_app_mentions
from dzeck.webui.gateway_services import GatewayServices
...
from dzeck.webui.mcp_presets_api import normalize_mcp_preset_mentions
from dzeck.webui.transcript import append_transcript_object, build_user_transcript_event
from dzeck.webui.websocket_logging import websockets_server_logger
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      Dzeck and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from dzeck.utils.logging_bridge import redirect_lib_logging
