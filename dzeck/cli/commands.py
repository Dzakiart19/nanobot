...
# Remove default handler and re-add with unified Dzeck format
logger.remove()
...
from dzeck.config.schema import Config  # noqa: E402
from dzeck.utils.evaluator import evaluate_response  # noqa: E402
from dzeck.utils.helpers import sync_workspace_templates  # noqa: E402
from dzeck.utils.restart import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} Dzeck - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from dzeck.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} dzeck[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} dzeck[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} Dzeck v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize Dzeck configuration and workspace."""
    from dzeck.config.loader import get_config_path, load_config, save_config, set_config_path
    ...
    if wizard:
        from dzeck.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'dzeck onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'dzeck agent -m "Hello!"'
    gateway_cmd = "dzeck gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} Dzeck is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDSLab/dzeck#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from dzeck.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from dzeck.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from dzeck.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from dzeck.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show Dzeck runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'dzeck-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from dzeck.api.server import create_app
    from dzeck.bus.queue import MessageBus
    from dzeck.providers.image_generation import image_gen_provider_configs
    from dzeck.session.manager import SessionManager

    if verbose:
        logger.enable("dzeck")
    else:
        logger.disable("dzeck")
...
def _load_or_create_desktop_config(config: str | None, workspace: str | None) -> Config:
    ...
    from dzeck.config.loader import (
        get_config_path,
        load_config,
...
def desktop_gateway(
    ...
    """Start the private local gateway used by Dzeck Desktop."""
...
def _run_gateway(
    ...
    *,
    ...
    from dzeck.cron.types import CronJob
    from dzeck.providers.factory import build_provider_snapshot, load_provider_snapshot
    from dzeck.providers.image_generation import image_gen_provider_configs
    from dzeck.session.manager import SessionManager
    ...
    console.print(f"{__logo__} Starting Dzeck gateway version {__version__} on port {port}...")
    ...
    from dzeck.agent.loop import UNIFIED_SESSION_KEY
    ...
    from dzeck.cron.types import CronJob, CronPayload, CronSchedule
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show Dzeck runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from dzeck.channels.registry import discover_all
    from dzeck.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from dzeck.channels.registry import discover_all, discover_channel_names
    from dzeck.config.loader import load_config
...
def status():
    """Show Dzeck status."""
    from dzeck.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} Dzeck Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from dzeck.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from dzeck.providers.github_copilot_provider import login_github_copilot
