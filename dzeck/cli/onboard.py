"""Interactive onboarding questionnaire for dzeck."""
...
def _show_main_menu_header() -> None:
    ...
    from Dzeck import __logo__, __version__
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]dzeck[{__version__}][/bold cyan]")
    )
    console.print()
...
def _handle_fallback_models_field(
    ...
    from dzeck.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from dzeck.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"dzeck.channels.{name}")
