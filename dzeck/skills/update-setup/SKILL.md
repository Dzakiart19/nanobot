---
name: update-setup
description: One-time setup wizard for the Dzeck upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `Dzeck --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v Dzeck || true
python -m pip show Dzeck-ai || true
pipx list | sed -n '/Dzeck-ai/,+3p' || true
uv tool list | sed -n '/Dzeck-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how Dzeck was
installed.
...
`question: "Where is your Dzeck source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "Dzeck-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "Dzeck-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "Dzeck-ai[EXTRAS]"` |
...
4. Use `exec` to verify: `Dzeck --version`
5. Tell the user the new version. Say: "Run `/restart` to restart Dzeck and apply the update. If `/restart` is unavailable in this channel, restart the Dzeck process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade Dzeck' when you want to update."
