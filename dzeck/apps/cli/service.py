...
from dzeck.apps.protocol import app_manifest, compact_dict
from dzeck.config.paths import get_runtime_subdir
from dzeck.security.workspace_policy import is_path_within
...
DZECK_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/dzeck-extension/main/registry.json"
DZECK_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/dzeck-extension/main"
_CATALOG_SOURCES = (
    ...
    ("extensions", DZECK_EXTENSION_REGISTRY_URL, DZECK_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "dzeck-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "dzeck-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["dzeck_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from dzeck."
        return f"""---
...
Use this skill when the user asks Dzeck to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_dzeck_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- dzeck-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_dzeck_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from dzeck. {entry_point} "
                "is still available because it is managed outside dzeck."
            )
