...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses Dzeck application-level guards.",
    )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("DZECK_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("DZECK_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("DZECK_SANDBOX_ENFORCED")
