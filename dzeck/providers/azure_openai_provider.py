...
helpers from :mod:`dzeck.providers.openai_responses`.
...
2. **Microsoft Entra ID (AAD)** — when ``api_key`` is empty the provider
   ...
   is an optional dependency installed via ``pip install dzeck-ai[azure]``.
"""
    ...
    def __init__(self, scope: str = _AZURE_OPENAI_SCOPE) -> None:
        try:
            ...
            raise RuntimeError(
                ...
                "Install it with: pip install 'dzeck-ai[azure]'"
            ) from exc
