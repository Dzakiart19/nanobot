...
from dzeck.config.schema import Config, _resolve_tool_config_refs
...
def _migrate_legacy_dir() -> None:
    """Migrate ~/.nanobot to ~/.dzeck if the old directory exists and the new one doesn't."""
    import shutil
    old_dir = Path.home() / ".nanobot"
    new_dir = Path.home() / ".dzeck"
    if old_dir.exists() and not new_dir.exists():
        try:
            shutil.copytree(str(old_dir), str(new_dir))
            logger.info("Migrated {} → {}", old_dir, new_dir)
        except Exception as exc:
            logger.warning("Could not migrate {} → {}: {}", old_dir, new_dir, exc)
...
def get_config_path() -> Path:
    ...
    _migrate_legacy_dir()
    return Path.home() / ".dzeck" / "config.json"
