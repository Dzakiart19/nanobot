...
Dzeck can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
pip install "Dzeck-ai[api]"
Dzeck serve
```
...
If `channel` points to a channel that is not enabled in your config, Dzeck will queue the outbound event but no platform delivery will occur.
