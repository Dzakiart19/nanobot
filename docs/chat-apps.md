...
Connect Dzeck to your favorite chat platform. Want to build your own? See the [Channel Plugin Guide](./channel-plugin-guide.md).
...
a public HTTPS URL that forwards to Dzeck's local listener and set `mode` to
`webhook`:
...
> tunnel in front of it. TLS/Host policy is handled by your proxy; Dzeck only
> listens on `webhookListenHost:webhookListenPort` and validates Telegram's
> webhook secret token. `webhookMaxConnections` defaults to `4`; Dzeck
...
> `webhookPath` is the local path Dzeck listens on. They often use the same
...
**1. Ask Dzeck to set up Mochat for you**

Simply send this message to Dzeck (replace `xxx@xxx` with your real email):
...
- You need:
  - `userId` (example: `@Dzeck:matrix.org`)
  - `password`
...
- **Permissions**:
  ...
  - **Streaming replies** (default in Dzeck): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
  - If you **cannot** add `cardkit:card:write`, set `"streaming": false` under `channels.feishu` (see below). The bot still works; replies use normal interactive cards without token-by-token streaming.
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running Dzeck first to establish connection)
...
> `allowFrom`: Add your open_id (find it in Dzeck logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in Dzeck logs when you message the bot). Use `["*"]` for public access.
...
> `files:read` is required to read files users send to Dzeck. `files:write` is required for Dzeck to send images, videos, and other file uploads. If you add either scope later, reinstall the Slack app to the workspace and restart Dzeck so it uses the updated bot token.
...
Give Dzeck its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `my-Dzeck@gmail.com`)
...
```json
{
  "channels": {
    "email": {
      ...
      "imapPort": 993,
      "imapUsername": "my-Dzeck@gmail.com",
      "imapPassword": "your-app-password",
      "smtpHost": "smtp.gmail.com",
      "smtpPort": 587,
      "smtpUsername": "my-Dzeck@gmail.com",
      "smtpPassword": "your-app-password",
      "fromAddress": "my-Dzeck@gmail.com",
...
> - `allowFrom`: Add the sender ID you see in Dzeck logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and Dzeck will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, Dzeck will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to Dzeck's runtime directory for Weixin state.
