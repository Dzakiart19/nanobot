...
Build a custom Dzeck channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of Dzeck (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.
...
```text
Dzeck-channel-webhook/
├── Dzeck_channel_webhook/
...
```python
# Dzeck_channel_webhook/__init__.py
from Dzeck_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# Dzeck_channel_webhook/channel.py
import asyncio
...
from Dzeck.config.schema import Base
...
# pyproject.toml
[project]
name = "Dzeck-channel-webhook"
version = "0.1.0"
dependencies = ["Dzeck-ai", "aiohttp"]

[project.entry-points."Dzeck.channels"]
webhook = "Dzeck_channel_webhook:WebhookChannel"
...
packages = ["Dzeck_channel_webhook"]
```
...
Besides normal assistant text, Dzeck can emit low-emphasis trace blocks. These are intended for UI affordances like status rows, collapsible "used tools" groups, or reasoning/thinking blocks. Platforms that do not have a good place for them can ignore them safely.
...
Built-in channels use Pydantic config models (subclassing `Base` from `Dzeck.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `Dzeck.config.schema.Base`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `Dzeck-channel-{name}` | `Dzeck-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `Dzeck_channel_{name}` | `Dzeck_channel_webhook` |
